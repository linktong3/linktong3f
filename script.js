...
const originalInformation = [
    ...[`Distance traveled:`, '*7.9 billion kms', '^'],
    ...[`Years in space:`, '*20', '-', '^'],
    ...[`Saturn orbits:`, '*294', '^'],
    ...[`Images Taken:`, '*453_048', '-', '^'],
    ...[`Commands executed:`, '*2.5 million', '^'],
    ...[`Data transmitted:`, '*635 GB', '-', '^'],
    ...[`Research papers published:`, '*~4000', '^'],
    ...[`Named moons discovered:`, '*6', '-', '^'],
    ...[`Nations participated:`, '*27', '-', '^'],
    ...maybeKeepLongMessage([
        `Cassini-Huygens was the first missiong to:^*- orbit Saturn^*- land in the outer solar system^*- sample an extraterrestrial ocean`,
        '-', '^',
        `It revealed Titan to be one of the most Earth-like worlds we’ve encountered`,
        '-', '^',
        `(Titan is one of Saturn’s moons)`,
        '-', '^',
    ]),
    ...maybeKeepLongMessage([
        cpxaisfm(['see NASA\'s website=', { tag: 'a', attributes: { href: 'https://solarsystem.nasa.gov/missions/cassini/overview/', target: '_blank' } }, 'here', { end: true }, `=for more details`]),
        '-', '^',
        cpxaisfm(['for 3d data from NASA=', { tag: 'a', attributes: { href: 'https://nasa3d.arc.nasa.gov/models', target: '_blank' } }, 'go here', { end: true }, `.`]),
        '-', '^',
    ]),
    cpxaisfm(['see my=', { tag: 'a', attributes: { href: 'https://codepen.io/lukes611', target: '_blank' } }, 'codepen profile', { end: true }, '=for more pens']),
    '-', '^',
    ...maybeKeepLongMessage([cpxaisfm([
        'follow me on=', { tag: 'a', attributes: { href: 'https://twitter.com/Lukes611', target: '_blank' } }, 'twitter', { end: true }, '=or=', { tag: 'a', attributes: { href: 'https://www.instagram.com/sip.wave', target: '_blank' } }, 'Instagram', { end: true }, '=for updates / other creative work']),
        '-', '^',
        '-', '^',
        'humans can do a lot',
        '-', '^',
        'we just need to care about each other, and trust science',
    ]),
    '-', '^',
    'Thanks for viewing',
].map(x => x);
